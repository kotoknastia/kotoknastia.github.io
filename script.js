...
function init() {
    ...
    $("name").onchange = nameOnChange;
    $("mail").onchange = emailOnChange;
    $("next").onclick = nextOnClick;
    $("prev").onclick = prevOnClick;
    $("country").onchange = countryOnChange;
    $("city").onchange = cityOnChange;
    $("fb").onchange = fbChecked;
    $("vk").onchange = vkChecked;
    $("twit").onchange = twitChecked;
    $("ok").onchange = okChecked;
    $("fbChecked").onchange = fbOnChange;
    $("vkChecked").onchange = vkOnChange;
    $("twitChecked").onchange = twitOnChange;
    $("okChecked").onchange = okOnChange;
...
function finish() {
	...
	addItems();
	steps(4);
	document.body.background = "linear-gradient(to bottom right, white, #f5f5f5);"
}
...
function buttons(count) {
	var buttons = {"one" : "able", "two": "disable", "three": "disable", "four": "disable"};
	var i = 0;
	for (key in buttons) {
		if (i == 0) {
			if (ableBtn > 0 && i == count) {
